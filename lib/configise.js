...
if(!_.isEmpty(derived_object)) {
  _.pairs(derived_object).each(function(pair) {
    var key = pair[0];
    var derive_function = pair[1];
    config_object[key] = derive_function(config_object);
  });
}
...
if(!_.isEmpty(verify_object)) {
  ...
  var missing_keys = _.functions(verify_object).
    filter(_.partial(_.has, verify_object)). // No prototype keys
    reject(_.partial(_.has, config_object));
  if(!_.isEmpty(missing_keys)) {
    throw new Error("Missing configuration keys: [" + missing_keys.join(", ") + "]");
  }

  // Test the keys
  var failure_keys = _.functions(verify_object).
    filter(_.partial(_.has, verify_object)). // No prototype keys
    reject(function(key) {
      verify_object[key](configuration_object[key]);
    });
  if(!_.isEmpty(failure_keys)) {
    throw new Error("Failed verification keys: [" + failure_keys.join(", ") + "]");
  }
}
