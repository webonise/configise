...
//check for "' warn user and remove it
_(["USER", "ENV", "NODE_ENV"]).each(function(env) {
  if ((/['||"]/g).test(process.env[env])) {
    console.warn("found quotes around " + env + " removing it");
    process.env[env].replace(/['||"]/, "");
  }
});
...
file_names = _(file_names).map(function(it) {
  return it.toLowerCase() + ".js";
});

// Basic workhorse method to load an object based on a directory
var load_object = function(obj, dir) {
  var to_obj = function(file_name) {
    return optional(dir + "/" + file_name);
  };
...
if (!_.isEmpty(derived_object)) {
  ...
  _.each(pairs, function(pair) {
    ...
    try {
      ...
      err.message = "Error processing derivation of key " + key + ": " + err.message + " => " + util.inspect(derive_function, {
        depth: 0
      });
      throw err;
    }
...
if (!_.isEmpty(verify_object)) {
  ...
  failure_keys = _.reject(failure_keys, function(key) {
    try {
      ...
      err.message = "Error processing validation of key " + key + ": " + err.message + " => " + util.inspect(verify_object[key], {
        depth: 0
      });
      throw err;
    }
